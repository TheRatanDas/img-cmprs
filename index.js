...
var dropBody = document.querySelector('.main');
let completed = 0;
...
var animatingRocket = false;
var woofyIdleInterVal, woofyEarsInterVal, woofyEarsRInterVal;
var isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
let quality = 0.70;
let maxWH = false;
let webpConversion = 0;
let imagePrefix = '';
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.addEventListener('compression_start', function(e) {
        if (woofyIdleInterVal) {
            clearInterval(woofyIdleInterVal)
        };
        if (woofyEarsInterVal) {
            clearInterval(woofyEarsInterVal)
        };
        if (woofyEarsRInterVal) {
            clearInterval(woofyEarsRInterVal)
        };
    ...
    document.addEventListener('compression_complete', function(e) {
        if (animatingRocket === false) {
            ...
            var rocket = document.querySelector('.content_animation_rocket');
            setTimeout(() => {
...
function handleDrop(e) {
    ...
    if (files) {
        ...
        if (NoImages) {
            var compressErrorEvent = new CustomEvent('compression_error', {
                detail: {
                    error: 'Only JPG & PNG Files Please!'
                }
            });
...
function processFiles(files) {
    const promises = [];
    var compressStartEvent = new CustomEvent('compression_start');
        ...
        if (file.type === 'image/jpg' || file.type === 'image/jpeg' || file.type === 'image/png') {
            reader.addEventListener("load", function(event) {
                var newFile = {
                    id: UID,
                    fileName: file.name,
                    size: parseFloat(file.size.toFixed(1)),
                    sizeAfter: 0,
                ...
                renderFile(newFile);
                if (webpConversion === 1) {
                    ConvertToWebP(event.target.result).then((compressedIMGBlob) => {
...
function updateStatus() {
    ...
    updateElementStatus(completed);
    if (total === converted) {
        var completeEvent = new CustomEvent('compression_complete');
...
function updateElementStatus(completed) {
    if (completed < 101) {
        ...
        let totalAfterCompress = 0;
        uploadedFiles.forEach((item) => {
            totalUploadSize = totalUploadSize + item.size;
        })
        Object.keys(compressedFiles).forEach((itemKey) => {
            totalAfterCompress = totalAfterCompress + compressedFiles[itemKey].sizeAfter;
        });
        ...
        document.querySelector('.fileList_footer__totalSaving').innerHTML = 'Total Reduced ' + readableFileSize(totalSaved, true) + ' <span>(' + compressedPercentR + '%)</span>';
...
function fileHTML(file, done = false) {
    var compressedPercent = done ? Math.round(file.sizeAfter / file.size * 100) - 100 : 0;
    var doneIcon = done ? '<span class="file_done_icon"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" focusable="false" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 64 64"><circle cx="32" cy="32" r="30" fill="#4bd37b"/><path fill="#fff" d="M46 14L25 35.6l-7-7.2l-7 7.2L25 50l28-28.8z"/><rect x="0" y="0" width="64" height="64" fill="rgba(0, 0, 0, 0)" /></svg></span>' : '';
    var downloadButton = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" focusable="false" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 8zM4 19h16v2H4z" fill="#999999"/></svg>';
    var viewIcon = '<svg onclick="compareImages(\'' + file.id + '\')" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" focusable="false" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path d="M16.325 14.899l5.38 5.38a1.008 1.008 0 0 1-1.427 1.426l-5.38-5.38a8 8 0 1 1 1.426-1.426zM10 16a6 6 0 1 0 0-12a6 6 0 0 0 0 12zm3-5h-2v2a1 1 0 0 1-2 0v-2H7a1 1 0 0 1 0-2h2V7a1 1 0 1 1 2 0v2h2a1 1 0 0 1 0 2z" fill="#ffffff" fill-rule="evenodd"/><rect x="0" y="0" width="24" height="24" fill="rgba(0, 0, 0, 0)" /></svg>';
...
function generateFileName(fileName, fileType) {
    ...
    if (imagePrefix) {
        newFileName = newFileName.replace('.jpg', imagePrefix + '.jpg').replace('.jpeg', imagePrefix + '.jpeg').replace('.png', imagePrefix + '.png');
    }
...
function compareImages(fileID) {
    ...
    if (file && file.newImage) {
        document.querySelector('.before_image').innerHTML = '<img src="' + file.image + '" />';
        document.querySelector('.after_image').innerHTML = '<img src="' + window.URL.createObjectURL(file.newImage) + '" />';
        document.querySelector('.before_image').dataset.size = "Before (" + readableFileSize(file.size, true) + ")";
        document.querySelector('.after_image').dataset.size = "After (" + readableFileSize(file.sizeAfter, true) + ")";
        setTimeout(() => {
            var natWidth = document.querySelector('.after_image img').naturalWidth;
            var natHeight = document.querySelector('.after_image img').naturalHeight;
            if (natHeight > natWidth && natHeight > (window.innerHeight * 90 / 100)) {
                document.getElementById('compare').classList.add('compare--portrait');
            }
            if (natWidth < 1200 && natHeight < (window.innerHeight * 90 / 100)) {
                document.getElementById('compare').classList.add('compare--imageHeight');
                document.querySelector('.compare__inner').style.width = natWidth + 'px';
            }
            document.getElementById('compare').classList.add('compare--show');
        }, 100);
    }
}
...
function closeCompare() {
    ...
    document.querySelector('.before_image').innerHTML = '';
    document.querySelector('.before_image').dataset.size = "Before (0 Kb)";
    document.querySelector('.after_image').innerHTML = '';
    document.querySelector('.after_image').dataset.size = "After (0 Kb)";
}

function showContact() {
    document.getElementById('contact_modal').classList.add('contact_modal--show');
    document.querySelector('.contact__inner').innerHTML = '<iframe src="https://my.nativeforms.com/QjakV1dp1jZmQVQmBlWL1Db" width="100%" height="600" frameborder="0" ></iframe>';
}

function closeContact() {
    document.getElementById('contact_modal').classList.remove('contact_modal--show');
    document.querySelector('.contact__inner').innerHTML = '';
}
...
function woofyIdle() {
    woofyIdleInterVal = setInterval(function() {
        var eyes = document.querySelector('.woofy__eyes');
        var eyes_closed = document.querySelector('.woofy__eyes_closed');
        eyes.style.display = 'none';
        eyes_closed.style.display = 'block';
        setTimeout(() => {
            eyes.style.display = 'block';
            eyes_closed.style.display = 'none';
        }, 200);
    }, 3600);
    woofyEarsInterVal = setInterval(function() {
        var ears_left = document.querySelector('.woofy__ears_left');
        ears_left.style.backgroundPosition = '-326px -385px';
        setTimeout(() => {
            ears_left.style.backgroundPosition = '-550px -385px';
        }, 3500);
    }, 6200);
    woofyEarsRInterVal = setInterval(function() {
        var ears_right = document.querySelector('.woofy__ears_right');
        ears_right.style.backgroundPosition = '-432px -385px';
        setTimeout(() => {
            ears_right.style.backgroundPosition = '-655px -385px';
        }, 3500);
    }, 9200);
}
    ...
    if (compressedIMGBlob) {
        ...
        if (!compressedFiles[UID]) {
            const updatedFile = { ...newFile,
                newImage: compressedIMGBlob,
                sizeAfter: parseFloat(compressedIMGBlob.size.toFixed(1)),
                converted: true
            };
